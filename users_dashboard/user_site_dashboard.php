...
if ($_SESSION['role'] != 3) { // Vérifie que l'utilisateur a le rôle de site
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $description = "Demande pour " . $item . " - " . $objet;
    $commande_id = null;
    $fermer_commande = isset($_POST['fermer_commande']) ? 1 : 0; // Nouveau champ pour fermer la commande

    // Créer une nouvelle commande avec notification à logistique
    $sql_commande = "INSERT INTO commandes (date_commande,description, notified) VALUES (?, ?, ?)";
    $stmt_commande = $conn->prepare($sql_commande);
    $stmt_commande->bind_param("ssi", $date_saisie, $description,$fermer_commande);
    $stmt_commande->execute();
    $commande_id = $stmt_commande->insert_id; // Récupérer l'ID de la commande nouvellement créée
    
    // Insérer la demande d'expression de besoins avec le commande_id
    $sql = "INSERT INTO expression_besoins (date_saisie, item, objet, piece, image, nombres_articles, urgence, type_services, commande_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    header("Location: user_site_dashboard.php");
    exit();
}
?>
...
<head>
    ...
    <title>User Site Dashboard</title>
